...
function DaySection({day,expanded,onToggle,onClickAct,onOpenDay,onAdd}){
  const dc=day.activities.reduce((s,a)=>s+pJ(a.cost,a.currency),0);
  return(<div style={S.daySection}>
    <div style={S.dayHeader} onClick={onToggle}><div style={S.dayNum}>{String(day.id).padStart(2,"0")}</div><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><span style={{fontSize:15,fontWeight:700,color:"#fff"}}>{day.dow}, {day.date}</span><span style={S.locBadge}>{day.location}</span><span style={{fontSize:12,color:"rgba(255,255,255,0.45)"}}>{day.weather}</span></div><div style={{fontSize:12,color:"rgba(255,255,255,0.5)",marginTop:2}}>{day.theme} · {day.activities.length} exp{dc>0?` · ~${fJ(dc)}`:""}
    </div></div><button onClick={e=>{e.stopPropagation();onOpenDay(day.id);}} style={S.mapBtn} title="日视图+地图">🗺️</button><div style={{fontSize:18,color:"rgba(255,255,255,0.4)",transform:expanded?"rotate(180deg)":"",transition:"transform 0.2s"}}>▾</div></div>
    {expanded&&<div style={{padding:"8px 14px 14px"}}>{day.activities.map(a=><ActivityCard key={a.id} act={a} onClick={()=>onClickAct(a,day.id)}/>)}<button onClick={()=>onAdd(day.id)} style={S.addBtn}><span style={{fontSize:18}}>+</span> 添加体验</button></div>}
  </div>);
}
...
export default function App(){
  ...
  const cities=[];let prev="";days.forEach(d=>{const l=d.location.split("→")[0].split("/")[0].trim();if(l!==prev){cities.push({name:l,start:d.id});prev=l;}});
  const tA=days.reduce((s,d)=>s+d.activities.length,0);
  const dateRange=useMemo(()=>{if(!days.length)return"";const months={Jan:"1月",Feb:"2月",Mar:"3月",Apr:"4月",May:"5月",Jun:"6月",Jul:"7月",Aug:"8月",Sep:"9月",Oct:"10月",Nov:"11月",Dec:"12月"};const first=days[0].date,last=days[days.length-1].date;const[fm,fd]=first.split(" ");const[lm,ld]=last.split(" ");if(fm===lm)return`2026年${months[fm]}${fd}–${ld}日`;return`2026年${months[fm]}${fd}日–${months[lm]}${ld}日`;},[days]);
  ...
  return(<div style={S.root}>
    ...
    <div style={S.container}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4, 1fr)",gap:8,margin:"12px 0 16px"}}>
        {[["天数","7天"],["体验",tA],["城市",cities.length],["住宿","2处"]].map(([l,v])=>(<div key={l} style={S.sumItem}><span style={S.sumLabel}>{l}</span><span style={S.sumValue}>{v}</span></div>))}
      </div>
      {days.map(day=>(<DaySection key={day.id} day={day} expanded={!!exp[day.id]} onToggle={()=>setExp(p=>({...p,[day.id]:!p[day.id]}))} onClickAct={(a)=>hClick(a,day.id)} onOpenDay={id=>setDayM(id)} onAdd={did=>{setEditA({id:"new_"+Date.now(),type:"poi",name:"",category:"sightseeing",time:"Morning",cost:"",currency:"¥",notes:""});setEditD(did);}}/>))}
